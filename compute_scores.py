...
def main():
    ...
    with open('/home/yiren/lavis_datasets/vatex/annotations/cap_val.json', 'r') as f:
          anns = json.load(f)
    ...
    for ann in tqdm.tqdm(anns):
        if ann["image_id"] not in references.keys():
            references[ann["image_id"]] = [ann["caption"]]
        else:
            references[ann["image_id"]].append(ann["caption"])
    print(len(references))

    with open("/home/yiren/VATEX/lavis/output/BLIP-T/Caption_vatex_stage2/20230806064/result/val_epoch9.json", 'r') as f:
          cands = json.load(f)
    candidates = {}
    for cand in cands:
        candidates[cand["image_id"]] = cand["caption"]
