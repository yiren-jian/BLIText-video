...
@registry.register_model("darkformer_t5")
class DarkformerT5(Blip2Base):

    PRETRAINED_MODEL_CONFIG_DICT = {
        "darkformer_flant5xl": "configs/models/blip2/blip2_pretrain_flant5xl.yaml",       ### TODO
        "darkformer_flant5xxl": "configs/models/blip2/blip2_pretrain_flant5xxl.yaml",     ### TODO
    }
    ...
    def __init__(
        ...
        self.Darkformer.text_proj = nn.Linear(self.Darkformer.config.hidden_size, self.embed_dim) # projection layer for self-contrastive learning

        self.Darkformer.temp = nn.Parameter(0.07 * torch.ones([]))
        ...
        self.Darkformer.t5_proj = nn.Linear(
            self.Darkformer.config.hidden_size, self.t5_model.config.hidden_size
        )
    ...
    def forward(self, samples):
        ...
        for t in text:
            words = t.split(" ")
            mid_point = random.randint(1, len(words)//2)
        ...
        text_feat = F.normalize(
            self.Darkformer.text_proj(text_output.last_hidden_state[:, 0, :]), dim=-1
        )
        ...
        sim_t2t = sim_t2t / self.Darkformer.temp

        rank = dist.get_rank()
        bs = text_feat.size(0)
        targets = torch.linspace(rank * bs, rank * bs + bs - 1, bs, dtype=int).to(
            self.Darkformer.device
        )
        ...
        inputs_t5 = self.Darkformer.cls_proj(text_output.last_hidden_state[:, 0, :])   ### 1x768 --> 1x(32x768)
        inputs_t5 = inputs_t5.reshape(bsz, n_query, hdim)   ### 1x(32x768) --> 1x32x768
        inputs_t5 = self.Darkformer.pooler(inputs_t5)
        inputs_t5 = self.Darkformer.t5_proj(inputs_t5)  ### 768 --> 2560
